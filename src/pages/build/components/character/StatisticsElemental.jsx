  ...
  return (
    <div className="flex h-max w-72 flex-col">
      ...
      <div className="mb-1.5 flex items-center gap-3 border-b border-white/10 pb-1.5">
        <span className="shrink-0 text-[10px] font-medium uppercase tracking-[0.15em] text-white/35">Elemental & Status</span>

        <div className="h-px flex-1 bg-white/5" />

        <div className="flex gap-4 text-[8px] uppercase tracking-[0.12em] text-white/20">
          <span>Dmg</span>
          <span>Res</span>
          <span>%</span>
        </div>
      ...
      <div className="flex flex-col">
        {elementalStats.map((stat) => (
          <div key={stat.id} className="flex h-7 items-center border-b border-white/5">
            <div className="flex min-w-0 flex-1 items-center gap-2">
              <span className="flex size-5 shrink-0 items-center justify-center">
                <img src={stat.src} alt="" draggable={false} className="size-3.5 object-contain" />
              </span>

              <span className="truncate text-[10px] text-white/50">{stat.name}</span>
            </div>

            <div className="flex items-center gap-4 font-mono text-[10px] tabular-nums text-white/70">
      ...
      <div className="mt-4 border-t border-white/10 pt-3">
        <div className="mb-2 flex items-center gap-2">
          <span className="text-[9px] font-medium uppercase tracking-[0.18em] text-white/30">Simulación</span>

          <div className="h-px flex-1 bg-white/5" />
        </div>

        <div className="space-y-1.5 text-[9px] leading-4 text-white/35">
